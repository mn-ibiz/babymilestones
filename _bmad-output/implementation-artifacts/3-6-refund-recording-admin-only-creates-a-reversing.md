...
- [ ] Task 1: Implement refund posting (AC: #1, #2, #4)
  - [ ] Add a refund function to `packages/wallet` accepting `{ originalEntryId, amount, reasonCode, note, postedBy }`; validate `reasonCode` required and amount ≤ remaining-refundable on the original (sum prior refunds against the original to support partial refunds).
  - [ ] Insert a `wallet_ledger` row with `kind='refund'`, `reverses_entry_id` = original ID, via the idempotent `post()` path; flag `loyalty_clawback_pending=true` on the entry.
- [ ] Task 2: Admin-only API endpoint (AC: #5)
  - [ ] Add a refund route under `apps/api/src/routes/` guarded by role check (`packages/auth`) permitting only `admin` and `super_admin`.
- [ ] Task 3: SMS-stub notification (AC: #3)
  - [ ] Queue a parent notification via `packages/sms` stub adapter on successful refund.
- [ ] Task 4: Audit (DoD)
  - [ ] Write the refund action to `audit_outbox`.
- [ ] Task 5: Tests (all)
  - [ ] Tests-first: reversing entry created with correct `kind`/`reverses_entry_id` (AC2); refund exceeding remaining-refundable rejected, partial refunds tracked (AC4); reason code required (AC1); Reception/non-admin roles rejected, admin/super_admin allowed (AC5); SMS-stub queued (AC3).
