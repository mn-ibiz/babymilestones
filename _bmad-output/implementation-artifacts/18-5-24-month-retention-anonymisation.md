...
- [ ] Task 1: Implement 24-month retention + anonymisation (AC: #1, #2, #3, #4)
  - [ ] Satisfy AC#1: Nightly job scans `attendances.observations` older than 24 months.
  - [ ] Satisfy AC#2: Strips `parent_id` and `child_id`; replaces names in free-text with `[child]`/`[parent]` using regex on first names.
  - [ ] Satisfy AC#3: Aggregate text retained for operational learning; PII cleared.
  - [ ] Satisfy AC#4: Job run + count logged.
  - [ ] Touch / create: `apps/jobs/anonymise/observations.ts`
- [ ] Task 2: Tests (AC: all)
